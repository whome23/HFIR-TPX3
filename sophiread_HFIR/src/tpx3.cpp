...
std::string Params::toString() const {
  ...
  ss << "ABS: radius=" << m_abs_radius 
     ...
     << ", max_cluster_size=" << m_abs_max_cluster_size
...
Params parseUserDefinedParams(const std::string& filepath){
  ...
  unsigned long int max_cluster_size = 1000;
  ...
  while (std::getline(user_defined_params_file,line)){
    ...
    if (name == "abs_radius"){
      ...
      ss >> min_cluster_size;
    } else if (name == "abs_max_cluster_size"){
      ss >> max_cluster_size;
  ...
  Params p(radius,min_cluster_size,max_cluster_size,spider_time_range);
